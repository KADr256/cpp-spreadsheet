	...
	namespace {
		...
		public:
			...
			double Evaluate(const SheetInterface& sheet) const override {
				double result;
				double left = lhs_.get()->Evaluate(sheet);
				double right = rhs_.get()->Evaluate(sheet);

				/*
				if (left == INFINITY || left == -INFINITY || right == INFINITY || right == -INFINITY) {
		...
		public:
			...
			double Evaluate(const SheetInterface& sheet) const override {
				...
				auto buf1 = sheet.GetCell(*cell_);
				if (buf1 == nullptr || buf1->GetText() == "") {
					return 0.0;
				}
				double result;
				auto buf2 = buf1->GetValue();
				...
				case 0:
				{
					try
					{
						if (std::get<std::string>(buf2)[0]<'0'|| std::get<std::string>(buf2)[0]>'9') {
							throw FormulaError(FormulaError::Category::Value);
						}
						size_t size;
						result = std::stod(std::get<std::string>(buf2),&size);
						if (size != std::get<std::string>(buf2).size()) {
							throw FormulaError(FormulaError::Category::Value);
						}
					}
				...
				case 1:
					result = std::get<double>(buf2);
					break;
				case 2:
					throw std::get<FormulaError>(buf2);
				}
